...
float glow(float x, float str, float dist) {
  // Hard cutoff beyond threshold so empty canvas pixels are strictly 0.0
  float raw = dist / pow(max(x, 1e-4), str);
  return smoothstep(0.015, 0.25, raw) * raw;
}
...
void main() {
  ...
  // If outside of thread glow, completely discard the pixel
  if (gsum < 0.001) {
    discard;
  }
  ...
  float alpha = clamp(gsum * 1.2, 0.0, 1.0) * uOpacity;
...
export default function WebThreads({
  ...
  return (
    <div
      ref={containerRef}
      className={`web-threads-container bg-transparent ${className}`.trim()}
    />
  );
}